...
// provided by MakeDefaultServerConfig [MakeDefaultServerConfig], sets the max thread count to 1; this guarantees FIFO processing od messages sent like in regular actor systems.
...
// It initializes the queue with a size of `1 + cfg.ThreadsCfg`.
func NewServer[T ServerHandler[M, O], M any, O any](handler T, cfg ServerConfig[T, M, O]) (*Server[T, M, O], error) {
	err := cfg.Validate()
	if err != nil {
		return nil, err
	}
	...
	return &Server[T, M, O]{
		queue:   queue.NewQueue(1 + int(cfg.ThreadsCfg), int(cfg.MailboxSize), makePolicy[ContextMessage[M, O]](cfg.MailboxPolicy)),
		handler: handler,
		sem:     make(chan struct{}, cfg.ThreadsCfg),
		recover: cfg.RecoverFn,
		generateUUIDs: cfg.GenerateUUIDs,
	}, nil
}
